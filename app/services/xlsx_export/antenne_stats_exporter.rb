  ...
  class AntenneStatsExporter < BaseExporter
    ...
    def xlsx
      ...
      wb.add_worksheet(name: I18n.t('antenne_stats_exporter.quarter_stats')) do |sheet|
        sheet.add_row ["#{@antenne.name} - #{@end_date.year}T#{TimeDurationService.find_quarter(@start_date.month, 'to_number')}"], style: @title
    ...
    #
    def create_styles(s)
      @title        = s.add_style bg_color: 'DD', sz: 16, b: true, alignment: { horizontal: :center, vertical: :center }
