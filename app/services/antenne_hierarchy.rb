class AntenneHierarchy
  ...
  def call
    if @antenne.national?
      ...
      @antenne.child_antennes = regional_antennes
      ...
      @antenne.child_antennes << local_antennes_without_regional
      ...
      @antenne.child_antennes = territorial_antennes
  ...
  def get_associated_antennes(targeted_territorial_level, antenne)
    Antenne.where(id: Antenne.not_deleted.where(institution_id: antenne.institution_id, territorial_level: targeted_territorial_level)
      .with_insee_codes(antenne.insee_codes).map(&:id))
  end
end
