  ...
  class MatchExporter < BaseExporter
    def fields
      # /!\ les fields de MatchExporter et SolicitationExporter doivent correspondre pour garantir la cohérence du fichier
      {
        ...
        company_forme_exercice: -> { company&.forme_exercice },
        inscrit_rcs: -> { I18n.t(company.inscrit_rcs, scope: [:boolean, :text]) },
        inscrit_rm: -> { I18n.t(company.inscrit_rm, scope: [:boolean, :text]) },
        activite_liberale: -> { I18n.t(company.activite_liberale, scope: [:boolean, :text]) },
