class CompaniesController < ApplicationController
  ...
  def needs
    ...
    @needs_in_progress = policy_scope(@facility.needs.diagnosis_completed)
      .merge(Match.in_progress)
      .order(created_at: :desc)
    @needs_done = policy_scope(@facility.needs.diagnosis_completed)
      .merge(Match.done)
      .order(created_at: :desc)

    email_needs = Need.for_emails(@facility.company.contacts.pluck(:email))

    @contact_needs_in_progress = policy_scope(email_needs.diagnosis_completed)
      .merge(Match.in_progress)
      .where.not(id: @needs_in_progress)
      .order(created_at: :desc)
    @contact_needs_done = policy_scope(email_needs.diagnosis_completed)
      .merge(Match.done)
