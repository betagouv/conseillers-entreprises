class SolicitationsController < PagesController
  ...
  before_action :filter_spam, only: [:update_step_contact, :update_step_company, :update_step_description]
  ...
  def retrieve_company_step_path
    @solicitation.landing_subject.fields_mode_siret? ? step_company_search_solicitation_path(@solicitation.uuid, anchor: 'section-breadcrumbs') : step_company_solicitation_path(@solicitation.uuid, anchor: 'section-breadcrumbs')
  end
  helper_method :retrieve_company_step_path
  ...
  def from_entreprendre_via_view_params?
    PartnerOrigin.from_entreprendre?(campaign: query_params[:mtm_campaign], kwd: query_params[:mtm_kwd]) && !(view_params[:redirected] == 'entreprendre')
  end
