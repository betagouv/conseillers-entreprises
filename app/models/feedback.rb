...
class Feedback < ApplicationRecord
  ...
  scope :user_antenne_region_eq, -> (region_code) { joins(user: { antenne: :territorial_zones }).where(users: { antennes: { territorial_zones: { regions_codes: [region_code] } } }) }
  ...
  def persons_to_notify
    users_and_experts_to_notify = if self.user.is_admin?
      self.need.matches.where(status: [:quo, :taking_care]).map(&:expert)
    else
