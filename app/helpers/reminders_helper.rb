module RemindersHelper
  ...
  def build_expert_reminders_link(expert)
    ...
    icon = tag.span(class: "fr-ml-1v reminders-icon #{t(current_basket, scope: 'reminders.experts.scopes.icon')}",
                    title: t('helpers.reminders.present_in_basket', basket: t(current_basket, scope: 'reminders.experts.scopes.name')),
                    'data-collection': current_basket,
                    'aria-hidden': 'true')
    link + icon
  end
  ...
  def with_action
    %i[poke last_chance refused quo_match taking_care_match refused_feedback starred_need]
  end
