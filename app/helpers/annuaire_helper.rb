module AnnuaireHelper
  ...
  def build_user_name_cell2(user, antenne)
    html = link_to(user[:name], admin_user_path(user[:id]), title: t('annuaire_helper.build_user_name_cell.edit_user', user_name: user[:name], antenne: antenne[:name]))
    ...
    html << tag.span(class: 'ri-nurse-fill blue fr-ml-1v', aria: { hidden: true }, title: t('annuaire_helper.build_user_name_cell.antenne_manager')) if user[:responsable]
    html << tag.span(class: 'ri-map-2-line blue fr-ml-1v', aria: { hidden: true }, title: t('annuaire_helper.build_user_name_cell.specific_territories')) if user[:has_specific_territories]
    html
  end
