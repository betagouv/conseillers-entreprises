module BreadcrumbsHelper
  ...
  def breadcrumbs_landing(landing, landing_theme, landing_subject = nil, params = {})
    ...
    if landing_subject.present?
      html << content_tag('li', link_to(landing_theme.title, landing_theme_path(landing, landing_theme, params), class: 'fr-breadcrumb__link'))
  ...
  def breadcrumbs_page(title)
    content_tag('li', link_to(t('breadcrumbs_helper.home_link.home'), root_path, class: 'fr-breadcrumb__link'))
  end
  ...
  def home_link(landing, params = {})
    if landing.iframe? && landing.integral_iframe?
      link_to(t('breadcrumbs_helper.home_link.pde'), landing, class: 'fr-breadcrumb__link')
    elsif landing.iframe?
      ''
    else
      params = params.permit(:landing_slug, :pk_campaign, :pk_kwd) if params.present?
      link_to(t('breadcrumbs_helper.home_link.home'), root_path(params), class: 'fr-breadcrumb__link')
    end
  end
end
