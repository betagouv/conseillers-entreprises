...
RSpec.describe ApiEntreprise::Entreprises do
  ...
  context 'SIREN number exists' do
    let(:token) { '1234' }
    let(:siren) { '123456789' }
    let(:url) { 'https://entreprise.api.gouv.fr/v2/entreprises/123456789?token=1234' }
  ...
  context 'SIREN is missing' do
    let(:token) { '1234' }
    let(:siren) { '' }
    let(:url) { 'https://entreprise.api.gouv.fr/v2/entreprises/?token=1234' }
  ...
  context 'Token is unauthorized' do
    let(:token) { '' }
    let(:siren) { '123456789' }
    let(:url) { 'https://entreprise.api.gouv.fr/v2/entreprises/123456789?token=' }
