...
describe ActivityReports::Generate::CooperationSolicitations do
  ...
  describe 'generate_files' do
    ...
    it 'create activity_report' do
      ...
      expect(ActivityReport.last.category).to eq('solicitations')
    end
  end
  ...
  describe 'destroy_old_files' do
    ...
    let!(:activity_report_ok) { create :activity_report, :category_solicitations, reportable: cooperation, start_date: 18.months.ago }
    let!(:activity_report_ko) { create :activity_report, :category_solicitations, reportable: cooperation, start_date: 3.years.ago }
    let!(:quarters) { described_class.new(cooperation).send(:last_periods) }
