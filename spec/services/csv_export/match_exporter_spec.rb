...
describe CsvExport::MatchExporter, CsvExport do
  ...
  let!(:company_satisfaction) { create :company_satisfaction, need: need, contacted_by_expert: true, useful_exchange: false, outcome_discover_support: true, comment: "Superchouette" }
  ...
  it do
    csv = <<~CSV
      ...
      #{I18n.l(date, format: :admin)},1234,Description,,,www.example.fr,aze123,,"#{landing_theme.title}",#{pde_subject.label},12345678900011,#{facility.readable_locality},Bretagne,Fossil,"SAS, société par actions simplifiée",4618Z,"",20 à 49 salariés,"","",Dennis Meadows,meadows@fossil.com,xx,,Mise en relation,#{I18n.l(date, format: :admin)},#{need.id},Valérie Masson-Delmotte,Environnement,Energie,#{match.id},#{expert.id},Christophe Cassou,GIEC Bretagne,GIEC,En cours,,,En cours de prise en charge,,,,http://test.host/besoins/#{need.id},oui,non,outcome_discover_support,Superchouette
    CSV
