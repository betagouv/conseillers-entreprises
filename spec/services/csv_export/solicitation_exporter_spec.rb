...
describe CsvExport::SolicitationExporter, CsvExport do
  ...
  it do
    csv = <<~CSV
      Date de sollicitation,Id de la sollicitation,Description de la sollicitation,Type de source,Intitulé de la source,Détail de la source,Id GA,Thème choisi sur le site,Sujet choisi sur le site,SIRET,Commune,Région de l’établissement,Nom de l’entreprise,Catégorie juridique,Code NAF,Effectifs,Company forme exercice,Inscrit rcs,Inscrit rm,Activite liberale,Contact en entreprise,Adresse mail du contact entreprise,Téléphone du contact entreprise,Tags de l’équipe,Statut de la sollicitation,Date de transmission du besoin,Identifiant du besoin,Conseiller,Thème réel,Sujet réel,Id de la mise en relation,Id de l'expert,Expert,Antenne de l’expert,Institution de l’expert,Statut de la mise en relation,Date de prise en charge,Date de cloture,Statut du besoin,Date d’archivage,Page besoin,Satisfaction - rappel,Satisfaction - utilité,Commentaire de satisfaction
      #{I18n.l(date, format: :admin)},1234,Description,,,,,"#{landing_theme.title}",#{pde_subject.label},12345678900011,22100,Région Bretagne,Fossil,Société commerciale,4618Z,20 à 49 salariés,,oui,non,non,Dennis Meadows,meadows@fossil.com,xx,,Mise en relation,#{I18n.l(date, format: :admin)},#{need.id},Valérie Masson-Delmotte,Environnement,Energie,#{match.id},#{expert.id},Christophe Cassou,GIEC Bretagne,GIEC,Pris en charge,,,En cours de prise en charge,,http://test.host/besoins/#{need.id},,,
    CSV
