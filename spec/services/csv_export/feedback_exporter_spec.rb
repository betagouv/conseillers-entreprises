...
describe CsvExport::FeedbackExporter, CsvExport do
  ...
  let!(:feedback1) { create :feedback, :for_need, feedbackable: need, user: user, description: 'Premier commentaire.', created_at: "Mon, 21 Jan 2024 16:42:46" }
  let!(:feedback2) { create :feedback, :for_need, feedbackable: need, user: user, description: 'Deuxième commentaire', created_at: "Tue, 23 Jan 2024 15:02:33" }
  ...
  it do
    csv = <<~CSV
      ...
      Iframe,landing-slug,www.example.fr,#{need.id},#{I18n.l(need.created_at, format: :admin)},12345678900011,Environnement,Energie,En cours de prise en charge,,En cours,,GIEC Bretagne,GIEC,"- 21/01/2024 16:42, Christophe Cassou: Premier commentaire.\n- 23/01/2024 15:02, Christophe Cassou: Deuxième commentaire",oui,non,Commentaire de satisfaction,http://test.host/besoins/#{need.id}
    CSV
