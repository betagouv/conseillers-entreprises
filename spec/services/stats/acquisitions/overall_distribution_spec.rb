...
# Characterization specs: lock the CURRENT output of the acquisitions graphs
# (line + percentage-column variants) so the single-query refactor is proven
# behaviour-preserving, including the deliberate overlap semantics
# (a need can count in several buckets; from_others is computed by subtraction).
describe 'Stats::Acquisitions overall distribution', type: :model do
  # Fixed window: January + February 2026 => 2 monthly columns.
  ...
  # Builds a diagnosis_completed need attached to a solicitation with the given
  # landing integration and form_info, backdated to `created_at`.
  ...
  it 'keeps the overlap semantics: from_others computed by subtraction, others first' do
    # need C matches BOTH Entreprendre and Iframes, inflating the others' sum, so
    # from_others absorbs plain need E and stays 0 — this quirk MUST be preserved.
